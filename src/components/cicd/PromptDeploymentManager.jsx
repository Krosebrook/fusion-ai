...
export function PromptDeploymentManager() {
  ...
  const deployPrompt = async (versionId, environment) => {
    try {
      await base44.entities.PipelineRun.create({
        ...
        environment,
        deployment_config: { prompt_version_id: versionId }
      });
  ...
  const triggerAutomatedTest = async (versionId) => {
    try {
      await base44.functions.invoke('testPrompts', {
            ...
            {promptVersions.slice(0, 10).map((version, idx) => (
              ...
              >
                ...
                <div className="flex gap-2 flex-wrap">
                  <Button
                    size="sm"
                    onClick={() => triggerAutomatedTest(version.id)}
                  ...
                  <Button
                    size="sm"
                    onClick={() => deployPrompt(version.id, 'dev')}
                  ...
                  <Button
                    size="sm"
                    onClick={() => deployPrompt(version.id, 'staging')}
                  ...
                  <Button
                    size="sm"
                    onClick={() => deployPrompt(version.id, 'production')}
